...
FritzOutletAccessory.prototype.setResetTotal = function(callback) {
		callback(null, 0);
};
...
//    }
	...
	//self.services.Outlet.getCharacteristic(FritzPlatform.Temperature).getValue(uTemp, undefined, FritzPlatform.Context);
    var service = this.services.Outlet;
	uTemp = service.FritzTemperatur;
    
    this.platform.log(`Updated ${this.type} ${this.ain} ${this.name}: `, uPower, 'W', uEnergy, 'kWh', service.FritzTemperatur, '°C');
    
	if (uPower !== undefined) {
		this.services.fakeGatoHistoryService.addEntry({ time: moment().unix(), power: uPower });
		//this.platform.log('logging: ', uPower);
	};
};
